...
import { isNotNull } from "../find-options/operator/IsNotNull";
                                    ...
                                    switch (findOp) {
                                        case "$not":
                                            if (parameterValue[findOp] === null) {
                                                operationsArray.push((_a = {}, _a[propertyPath] = isNotNull(), _a));
                                            }
                                            else {
                                                operationsArray.push((_b = {}, _b[propertyPath] = Not(parameterValue[findOp]), _b));
                                            }
                                            break;
                                        case "$lt":
                                            operationsArray.push((_c = {}, _c[propertyPath] = LessThan(parameterValue[findOp]), _c));
                                            break;
                                        case "$lte":
                                            operationsArray.push((_d = {}, _d[propertyPath] = LessThanEqual(parameterValue[findOp]), _d));
                                            break;
                                        case "$gt":
                                        case "$mt":
                                            operationsArray.push((_e = {}, _e[propertyPath] = MoreThan(parameterValue[findOp]), _e));
                                            break;
                                        case "$gte":
                                        case "$mte":
                                            operationsArray.push((_f = {}, _f[propertyPath] = MoreThanEqual(parameterValue[findOp]), _f));
                                            break;
                                        case "$any":
                                            operationsArray.push((_g = {}, _g[propertyPath] = Any(parameterValue[findOp]), _g));
                                            break;
                                        case "$eq":
                                            operationsArray.push((_h = {}, _h[propertyPath] = Equal(parameterValue[findOp]), _h));
                                            break;
                                        case "$in":
                                            operationsArray.push((_j = {}, _j[propertyPath] = In(parameterValue[findOp]), _j));
                                            break;
                                        case "$between":
                                            if (!Array.isArray(parameterValue[findOp]))
                                                continue;
                                            operationsArray.push((_k = {}, _k[propertyPath] = Between(parameterValue[findOp][0], parameterValue[findOp][1]), _k));
                                            break;
                                    }
                                }
