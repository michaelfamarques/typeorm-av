...
var Not_1 = require("../find-options/operator/Not");
var LessThan_1 = require("../find-options/operator/LessThan");
var LessThanEqual_1 = require("../find-options/operator/LessThanEqual");
var MoreThan_1 = require("../find-options/operator/MoreThan");
var MoreThanEqual_1 = require("../find-options/operator/MoreThanEqual");
var Any_1 = require("../find-options/operator/Any");
var Equal_1 = require("../find-options/operator/Equal");
var In_1 = require("../find-options/operator/In");
var Between_1 = require("../find-options/operator/Between");
var IsNotNull_1 = require("../find-options/operator/IsNotNull");
...
var QueryBuilder = /** @class */ (function () {
     ...
     */
    QueryBuilder.prototype.computeWhereParameter = function (where, indexWhere) {
        var _this = this;
        if (indexWhere === void 0) { indexWhere = 0; }
                ...
                andConditions = wheres.map(function (where, whereIndex) {
                    ...
                    return propertyPaths.map(function (propertyPath, propertyIndex) {
                        ...
                        if (propertyPath === "$or" || propertyPath === "$and") {
                            var parameterValue = where[propertyPath];
                            if (Array.isArray(parameterValue)) {
                                var andOrConditions = parameterValue.map(function (parameter) { return _this.computeWhereParameter(parameter, indexWhere + i++); })
                        ...
                        return columns.map(function (column, columnIndex) {
                            ...
                            var parameterName = "where_" + indexWhere + "_" + whereIndex + "_" + propertyIndex + "_" + columnIndex;
                            if (parameterValue === null) {
                                ...
                                for (var findOp in parameterValue) {
                                    switch (findOp) {
                                        case "$not":
                                            if (parameterValue[findOp] === null) {
                                                operationsArray.push((_a = {}, _a[propertyPath] = IsNotNull_1.isNotNull(), _a));
                                            }
                                            else {
                                                operationsArray.push((_b = {}, _b[propertyPath] = Not_1.Not(parameterValue[findOp]), _b));
                                            }
                                            break;
                                        case "$lt":
                                            operationsArray.push((_c = {}, _c[propertyPath] = LessThan_1.LessThan(parameterValue[findOp]), _c));
                                            break;
                                        case "$lte":
                                            operationsArray.push((_d = {}, _d[propertyPath] = LessThanEqual_1.LessThanEqual(parameterValue[findOp]), _d));
                                            break;
                                        case "$gt":
                                        case "$mt":
                                            operationsArray.push((_e = {}, _e[propertyPath] = MoreThan_1.MoreThan(parameterValue[findOp]), _e));
                                            break;
                                        case "$gte":
                                        case "$mte":
                                            operationsArray.push((_f = {}, _f[propertyPath] = MoreThanEqual_1.MoreThanEqual(parameterValue[findOp]), _f));
                                            break;
                                        case "$any":
                                            operationsArray.push((_g = {}, _g[propertyPath] = Any_1.Any(parameterValue[findOp]), _g));
                                            break;
                                        case "$eq":
                                            operationsArray.push((_h = {}, _h[propertyPath] = Equal_1.Equal(parameterValue[findOp]), _h));
                                            break;
                                        case "$in":
                                            operationsArray.push((_j = {}, _j[propertyPath] = In_1.In(parameterValue[findOp]), _j));
                                            break;
                                        case "$between":
                                            if (!Array.isArray(parameterValue[findOp]))
                                                continue;
                                            operationsArray.push((_k = {}, _k[propertyPath] = Between_1.Between(parameterValue[findOp][0], parameterValue[findOp][1]), _k));
                                            break;
                                    }
                                }
                                if (operationsArray.length == 0)
                                    return "";
                                operationsArray = operationsArray.map(function (parameter) { return _this.computeWhereParameter(parameter, indexWhere + i++); })
                                    .filter(function (expression) { return !!expression; });
